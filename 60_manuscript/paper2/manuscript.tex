...
\paragraph{Resolution.} The hydroxide was re-parameterised and the model re-validated; \textbf{all
four state points now pass at 1.0--1.8\,\%}. The change responsible was not the one the failure
first suggests: correcting the hydroxide \emph{size} to its published value is necessary but
insufficient, still leaving a 6--8\,\% overprediction, and scaling the ionic charges by 0.8 in the
spirit of the electronic continuum correction is what closes the gap. Water self-diffusivity, which
was not a gate criterion, recovers by 1.5--3.1$\times$ as an independent check.
\textbf{The tolerance was not widened, and a correction to the reference data made the reported
failure larger rather than smaller.} The full account, including every methodological change between
the failing and passing models and the residual discrepancies that qualify the pass, is given in the
companion paper, which this work cites for the molecular input it consumes.
